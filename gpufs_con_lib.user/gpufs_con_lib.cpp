/****
  * 2013
  * Experimental GPUfs library for controlling GPUfs character device
  */
		...
		const char *pathname, int flags, mode_t mode, char* o_flush)
{
	...
	s.input_is_owner= ((flags&O_WRONLY)|| (flags&O_RDWR)) >0;
	s.input_gpuid=gpuid;
	s.input_inode=inode_stat.st_ino;
	s.output_ownerid=gpuid;
...
int gpufs_drop_residence(int gpufs_dev_fd, int gpuid, unsigned long g_inode)
{
	...
	s.input_is_owner=0;
	s.input_gpuid=gpuid;
	s.input_inode=g_inode;
	s.output_ownerid=-5;
	int ret=0;
	
	ret=ioctl(gpufs_dev_fd,GPUFS_DEV_IOCTL_DROP_OWNER,&s);
	if (ret== EINVAL) {
		fprintf(stderr,"no owner found while dropping residence");
	}
...
int gpufs_file_close_stay_open(int gpufs_dev_fd, int gpuid, int file_fd){
	// do nothing
	return 0;
}
int gpufs_file_close(int gpufs_dev_fd, int gpuid, int file_fd)
{
	int ret=0;
/*
	if (drop_residence) {
